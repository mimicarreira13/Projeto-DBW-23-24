 ...
 */
export async function showInicioJogoGalo(req, res) {
  if (!req.isAuthenticated()) {
    return res.redirect("/criar_conta?message=not_authenticated");
  }

  res.render("JogoGaloInicio", {
 ...
 * @return {Function} res.redirect - Função para redirecionar o utilizador para outra página
 */
export async function showDificuldadeJogoGalo(req, res) {
  if (!req.isAuthenticated()) {
    return res.redirect("/criar_conta?message=not_authenticated");
  }

  res.render("JogoGaloDificuldade", {
    isAuthenticated: req.isAuthenticated(),
  });
}
 ...
 * @return {Function} res.redirect - Função para redirecionar o utilizador para outra página
 */
export async function showJogoGalo(req, res) {
  if (!req.isAuthenticated()) {
    return res.redirect("/criar_conta?message=not_authenticated");
  }
